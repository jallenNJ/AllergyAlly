...
router.post('/', async function(req, res, next){
    ...
    FoodWithIngredients = await WebScraper.StartScrape(FoodNames);

    returnArray = []
    for(Food in FoodWithIngredients){
        Ingreds = FoodWithIngredients[Food];
        for(Allergy of AllergyList) {
            if(!Ingreds.includes(Allergy)){
                returnArray.push(Food);
            }
        }
    }

    UreturnArray = returnArray.filter((val, index, self) => {
